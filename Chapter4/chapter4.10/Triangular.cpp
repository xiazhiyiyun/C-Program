...
void Triangular::display( int length, int beg_pos, ostream &os ){
    if(length < 0 || beg_pos + length > _max_elems){
        cerr << "err:display();ircorrect parameter." << endl;
    }
    os << "The Triangular sequence: " << endl;

    vector<int>::iterator it = _elems.begin() + (beg_pos - 1);
    vector<int>::iterator it_end = _elems.begin() + beg_pos + length - 1;
    while(it != it_end){
        os << *it << " ";
	it++;
    }
